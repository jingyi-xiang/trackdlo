...
def add_self_intersection_constraints (model, vars, dlo_diameter, Y, Y_cur, num_of_dlos=1, nodes_per_dlo=0):
    for i in range (0, len(Y)-1):
        for j in range (i, len(Y)-1):
            ...
            pA, pB, dist = shortestDistanceBetweenLines(Y_cur[i], Y_cur[i+1], Y_cur[j], Y_cur[j+1], True)
            # print('shortest dist between E({}, {}) and E({}, {}) is {}'.format(i, i+1, j, j+1, dist))
...
def post_processing (Y, Y_0, dlo_diameter, num_of_dlos=1, nodes_per_dlo=0):
    ...
    add_self_intersection_constraints(model, g_verts, dlo_diameter, Y_0, Y, num_of_dlos, nodes_per_dlo)
